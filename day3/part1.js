...
class Grid {
  ...
  draw(path) {
    ...
    path.forEach(instruction => {
      const [, dir, distance] = /(R|U|D|L)(\d+)/.exec(instruction)
      ...
      for (let i = 0; i < distance; i++) {
        ...
        if (this.points[coord] === LINE) {
          this.points[coord] = CROSS
          this.crossPoints.push({ x, y })
        } else {
          this.points[coord] = LINE
        }
      }
    })
  }
  ...
  getClosestCrossing() {
    ...
    this.crossPoints.forEach(point => {
...
const closest = grid.getClosestCrossing()
console.log(closest)
