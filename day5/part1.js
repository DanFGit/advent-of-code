...
const getOpcode = (opcode) => {
  ...
  const [
    ...
    c = POSITION_MODE,
    de
  ] = opcode < 9 ? [,,,,opcode] : String(opcode).match(/(\d|^)(\d|^)(\d|^)(\d\d)$/)

  // console.log(`Opcode: ${Number(de)}`)
  // console.log(`P1 mode: ${Number(c) === POSITION_MODE ? 'position' : 'immediate'}`)
  // console.log(`P2 mode: ${Number(b) === POSITION_MODE ? 'position' : 'immediate'}`)
  // console.log(`P3 mode: ${Number(a) === POSITION_MODE ? 'position' : 'immediate'}`)

  // console.log()
  return [Number(de), Number(c), Number(b), Number(a)]
}

const run = (intcode, pos) => {
  // console.log(intcode, pos)
  const [opcode, p1Mode, p2Mode, p3Mode] = getOpcode(intcode[pos])

  if (opcode === 99) return intcode

  const p1 = p1Mode === POSITION_MODE ? intcode[intcode[pos + 1]] : intcode[pos + 1]
  const p2 = p2Mode === POSITION_MODE ? intcode[intcode[pos + 2]] : intcode[pos + 2]
  const p3 = p3Mode === POSITION_MODE ? intcode[intcode[pos + 3]] : intcode[pos + 3]
  ...
  if (opcode === 3) {
    // console.log(`Writing ${1} to ${intcode[pos + 1]}`)
    intcode[intcode[pos + 1]] = 1 // hardcode the input
    return run(intcode, pos + 2)
  }

  if (opcode === 4) {
    // console.log(intcode)
    // console.log(`Reading ${intcode[pos + 1]}`)
    console.log(intcode[intcode[pos + 1]])
    return run(intcode, pos + 2)
  }
